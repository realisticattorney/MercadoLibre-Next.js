...
//SSR (Server-side rendering) to fetch all of the cates from that fake store API
  ...
  return (
    <div className="grid grid-flow-row-dense md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:-mt-40 lg:-mt-52 mx-auto">
      {categories.slice(0,4).map((cate) => (
        <cate key={cate.id} cate={cate} />
      ))}

    <img className="md:col-span-full" src="https://links.papareact.com/dyz" alt="" />
      
      <div className="md:col-span-2">
      {categories.slice(4,5).map((cate) => (
        <cate key={cate.id} cate={cate} />
      ))}
      </div>

      {categories.slice(5,-1).map((cate) => (
        <cate key={cate.id} cate={cate} />
      ))}
