...
const fulfillOrder = async (session) => {
  ...
  return app.firestore().collection('users',).doc(session.id).set({ amount: session.amount_total / 100, amount_shipping: session.total_details.amount_shipping / 100,
   images: JSON.parse(session.images),
